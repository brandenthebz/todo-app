...
async function go() {
  let client = new MongoClient("mongodb+srv://todoAppUser:hEyZUQHCcuEXlOgV@cluster0.f9bdp.mongodb.net/TodoApp?retryWrites=true&w=majority")
...
app.post("/update-item", async function (req, res) {
  let safeText = sanitizeHTML(req.body.text, {allowedTags: [], allowedAttributes: {}})
  await db.collection("items").findOneAndUpdate({ _id: new ObjectId(req.body.id) }, { $set: { safeText } })
  res.send("success")
})
